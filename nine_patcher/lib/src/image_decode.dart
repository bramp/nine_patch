...
// TODO(bramp): This currently depends on image/image. It would be useful to create
...
NinePatchMetadata decodeNinePatchMetadata(
  Image image, {
  String? name,
  double? scale,
}) {
...
/// Returns true if the pixel is not black or white.
bool isNotBlackOrWhite(Pixel p) => !(isBlack(p) || isWhite(p));

/// Returns true if the pixel is not black, white, or red.
...
/// Checks if the given image is a valid nine-patch image.
void checkIsNinePatch(Image image) {
  ...
  // TODO(bramp): Implement parsing the new "optical bounds", which are Red
  // pixels made up part of the content padding. The red can only be on the
  // outside, so the bottom row could look like:  RRRWWWWWWBBBBBBBRRRR
...
/// Finds the stretch area of the given nine-patch image.
Rectangle<int> findStretch(Image image) {
  ...
  return Rectangle.fromPoints(
    Point(stretchLeft, stretchTop),
    Point(stretchRight, stretchBottom),
  );
}

/// Finds the content padding of the given nine-patch image.
Rectangle<int> findContentPadding(Image image) {
  ...
  return Rectangle.fromPoints(
    Point(contentLeft, contentTop),
    Point(contentRight, contentBottom),
  );
}
